...
const int INVITE_CODE_LENGTH = 6;
const int INVITE_CODE_INDEX = 3;
const String INVITE_CODE_SPACER_CHAR = '-';
...
class PinCodeTextField extends StatefulWidget {
  ...
  const PinCodeTextField({
    Key key,
    this.maxLength: INVITE_CODE_LENGTH,
    this.controller,
    this.hideCharacter: false,
    this.highlight: true,
    this.highlightColor: AppTheme.primary,
    this.maskCharacter: ' ',
    this.spacerCharacter: INVITE_CODE_SPACER_CHAR,
    this.spacerIndex: INVITE_CODE_INDEX,
    ...
    this.hasError: false,
    this.errorBorderColor: AppTheme.error,
    this.onTextChanged,
  }) : super(key: key);

  @override
  State createState() => PinCodeTextFieldState();
}
...
class PinCodeTextFieldState extends State<PinCodeTextField> {
  ...
  void didUpdateWidget(
    PinCodeTextField oldWidget,
  ) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.maxLength < widget.maxLength) {
      setState(() => currentIndex = text.length);
  ...
  _calculatePinWidth() async {
    ...
    setState(() => pinWidth = tempPinWidth);
  }
  ...
  Widget _touchPinBoxRow() => GestureDetector(
